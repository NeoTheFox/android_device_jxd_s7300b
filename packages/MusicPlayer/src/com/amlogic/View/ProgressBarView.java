...
//		this.setFocusable(true);
	...
	public void init(){
		...
		Log.i("ProgressBarView","----------------------MenuItemName:"+MenuItemName);
		focus=Bitmap.createBitmap(bar, 0, 0, 10, 17);
		tail=Bitmap.createBitmap(bar, 0, 0, 4, 17);
	}
	...
	public void onDraw(Canvas canvas)
	{
		...
		if(newfocus != null)
			 newfocus.recycle();
		Log.i("ProgressBarView","------------------progress :"+progress);
		if(progress > 0)
		{
			newfocus= Bitmap.createScaledBitmap(focus, progress*94/10, 41, true); 
			int height=newfocus.getHeight();
			int weight=newfocus.getWidth();
			Log.i("ProgressBarView","------------------height :"+height);
			Log.i("ProgressBarView","------------------weight :"+weight);
