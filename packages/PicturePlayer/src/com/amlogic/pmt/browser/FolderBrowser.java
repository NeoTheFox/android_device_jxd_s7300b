	 ...
	 public List<FilebrowserItemData> GetFileList(String path)
	 {
		 ...
		 else
		 {
			 for (File file : files)
			 {
			 	if (file.isDirectory())
		 		{
			 		ItemList.add(new FilebrowserItemData(file.getPath(),SearchDid.SearchID("folder",true),SearchDid.SearchID("folder",false)));
		 		}
			 	else
			 	{
...
//			 		{
			 		ItemList.add(new FilebrowserItemData(file.getPath(),SearchDid.SearchID(FileType,true),SearchDid.SearchID(FileType,false)));
//			 		}
			 	}
		     } 	
	      }
