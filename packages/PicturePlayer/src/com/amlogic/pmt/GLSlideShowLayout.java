...
public class GLSlideShowLayout extends GLBaseLayout implements
	...
	private int currentRepeatMode = DataProvider.playmode_folder;
	private boolean currentRandomPlay = false;
	...
	GLSlideShowLayout(Context context, String name, String local) {
		...
		if (!data.equals("")) {
			...
			if (data.equals("SINGLE"))
				playMode = DataProvider.playmode_singer;
			else if (data.equals("FOLDER"))
				playMode = DataProvider.playmode_folder;
			else
				playMode = DataProvider.playmode_normal;
			currentRepeatMode = playMode;
		}
		this.dataProvider.setSwitchMode(currentRepeatMode);
	...
	private void switchSlot() {
		synchronized (slots) {
			...
			//we use random animations always
			int sm = (int) (Math.random() * (SwitchAnimParam.length - 1)) + 1;
		...
		//Make the rotation option back to "Normal"
		if (Menucontrol.initState != null) {
			for (int i = 0; i < Menucontrol.initState.size(); i++) {
				if (Menucontrol.initState.get(i).contains("shortcut_picture_rotate_")) {
					Log.d("GLSlideShowLayout", ">>>>>>>>>>Remove "+Menucontrol.initState.get(i));
					Menucontrol.initState.remove(i);
				}
			}
		}
	}
	...
	public void rotate(float rotation) {
		synchronized (slots) {
			...
			float ToAngle = rotation - currentRotation;
			if (ToAngle < -180)
			{
				ToAngle = rotation + 360;
			}
			else
			{
				ToAngle = rotation;
			}
			am1.setRotation(currentRotation, ToAngle, 0, 0, 1);
			animlist.add(am1);
			PMTAnimator.getInstance().startAnimations(animlist);
		}
	...
	private int delayAutoPlay = 3000;
