		...
		while (inLen > 0)
		{
			...
			if (ch < 0x80) {
				...
				*out++ = (UTF8)(0x80 | (ch&0x3F));
				...
				ucs4 = ((ch-0xD800)<<10) + (ch2-0xDC00) + 0x10000;
				if ((outMax -= 4) < 0) {
					return -1;
				}
...
static inline int subtitle_utf16_utf8(char *s, int inLen)
{
	...
	outLen = subtitle_uni_Utf16toUtf8((const UTF16 *)s, inLen/2, utf8_str, inLen*2);
	if(outLen > 0){
...
static inline int probe_utf8_utf16(char *probe_buffer, int size)
{
	...
	int i=0;
	for(i=0; i<(size-5); i++){
		if(probe_buffer[i] == 0 && (probe_buffer[i+1]>0x20 && probe_buffer[i+1]<0x7d)){
			i++;
			big_utf16++;
		}
		...
		else if(((probe_buffer[i]&0xE0)==0xE0)&&((probe_buffer[i+1]&0xC0)==0x80)&&((probe_buffer[i+2]&0xC0)==0x80)){
			i+=2;
			utf8_count++;
			if(((probe_buffer[i+1]>=0x80) && ((probe_buffer[i+1]&0xE0) != 0xE0))|| \
				((probe_buffer[i+2]>=0x80) && ((probe_buffer[i+2]&0xC0) != 0x80))|| \
				((probe_buffer[i+3]>=0x80) && ((probe_buffer[i+3]&0xC0) != 0x80)))
				return 0;
		}
...
static char *internal_subf_gets(char *s, int fd)
{
	...
	if(buffer_size <= 0)
		return NULL;
    while ((offset < buffer_size) && (!((subtitle_format==0||subtitle_format==1)&&(subfile_buffer[offset] == '\n'))
			&& !((subtitle_format==2)&&(subfile_buffer[offset]==0)&&(subfile_buffer[offset+1]==0xd)&&(subfile_buffer[offset+2]==0)&&(subfile_buffer[offset+3]==0xa))
			&& !((subtitle_format==3)&&(subfile_buffer[offset]==0xd)&&(subfile_buffer[offset+1]==0)&&(subfile_buffer[offset+2]==0xa)&&(subfile_buffer[offset+3]==0)))) {
        offset++;
    }
    ...
    else 
    {
        ...
        offset = 0;
        while ((offset <MIN(LINE_LEN - copied, buffer_size)) && (!((subtitle_format==0||subtitle_format == 1)&&(subfile_buffer[offset] == '\n'))
			&& !((subtitle_format==2)&&(subfile_buffer[offset]==0)&&(subfile_buffer[offset+1]==0xd)&&(subfile_buffer[offset+2]==0)&&(subfile_buffer[offset+3]==0xa))
			&& !((subtitle_format==3)&&(subfile_buffer[offset]==0xd)&&(subfile_buffer[offset+1]==0)&&(subfile_buffer[offset+2]==0xa)&&(subfile_buffer[offset+3]==0)))) {
            offset++;
        }
        if (((subtitle_format==0||subtitle_format == 1)&&subfile_buffer[offset] == '\n')
			||((subtitle_format==2)&&(subfile_buffer[offset]==0)&&(subfile_buffer[offset+1]==0xd)&&(subfile_buffer[offset+2]==0)&&(subfile_buffer[offset+3]==0xa))
			||((subtitle_format==3)&&(subfile_buffer[offset]==0xd)&&(subfile_buffer[offset+1]==0)&&(subfile_buffer[offset+2]==0xa)&&(subfile_buffer[offset+3]==0))) {
    ...
    do 
    {
        switch (state) 
        {
            ...
            if (*s == '\0') break;
                ...
                *p = '\0'; p = text; internal_trail_space (text);
                if (text[0] != '\0')
            ...
            if (q) {
                ...
                *p = '\0'; internal_trail_space (text);
                if (text[0] != '\0')
                    current->text.text[current->text.lines++] = internal_sub_strdup (text);
                if (current->text.lines > 0) { state = 99; break; }
                state = 0; continue;
            }
            s = strchr (s, '>');
...
SUBAPI subdata_t *internal_sub_open(char *filename,unsigned int rate,char* charset)
{
	...
	if(charset==NULL)
	{
		...
		else if((file_format[0]==0xEF)&&(file_format[1]==0xBB)&&(file_format[2]==0xBF))
		{
			subtitle_format = 1;
		}
		else
		...
		if(subtitle_format == 0){
			...
			if(probe_buffer != NULL){
				if(read(fd, probe_buffer, SUBTITLE_PROBE_SIZE) == SUBTITLE_PROBE_SIZE){
